import { Link } from "react-router-dom";
import Heroimg from "../image/ss9-1.png"

function Herosec() {
    return (
        <>
            <div classNameName="bg-white dark:bg-gray-800 flex relative z-20 items-center overflow-hidden">
                <div className="container mx-auto px-6 flex relative py-16">
                    <div className="sm:w-2/3 lg:w-2/5 flex flex-col relative z-20">
                        <span className="w-20 h-2 bg-gray-800 dark:bg-white mb-12">
                        </span>
                        <h1 className="font-bebas-neue uppercase text-6xl sm:text-8xl font-black flex flex-col leading-none dark:text-white text-gray-800">
                            Timely
                            <span className="text-5xl sm:text-7xl">
                                Delivery
                            </span>
                        </h1>
                        <p className="text-sm sm:text-base text-gray-700 dark:text-white mt-3">
                            Bringing the world of IT to your fingertips with the leading web development company in Jaipur. Experience seamless and efficient technology solutions tailored to your needs
                        </p>
                        <div className="flex mt-8">
                            <Link to="/contact" className="uppercase py-2 px-4 rounded-lg bg-grey-500 border-2 border-black-500 text-black text-md mr-4 hover:bg-black-400">
                                Get started
                            </Link>
                            <a href="#custom_form_sec" className="uppercase py-2 px-4 rounded-lg bg-white  border-2 border-black-500 text-white-500 dark:text-black hover:bg-black-500 hover:text-black text-md">
                                Read more
                            </a>
                        </div>
                    </div>
                    <div className="hidden sm:block sm:w-1/3 lg:w-3/5 relative">
                        <img src="https://www.tailwind-kit.com/images/object/10.png" className="max-w-xs md:max-w-sm m-auto" alt="hello" />
                    </div>
                </div>
            </div>
        </>
    );
}

export default Herosec;